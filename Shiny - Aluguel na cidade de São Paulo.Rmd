...
### Média das variáveis dos bairros
...
renderDataTable(datatable(compararBairros(dados, input$nomes_bairros),
                          options = list(pageLength = 10,lengthChange=FALSE,dom ='ltipr'),
                          escape=F,rownames = F) %>%
      formatStyle(columns = c(2:6), 'text-align' = 'center'))
```


### Previsão do aluguel gasto por área e bairro
...
renderDataTable(datatable(predBairros(dados, input$nomes_bairros, input$area),
                          options = list(pageLength = 10,lengthChange=FALSE,rownames = F,dom ='ltipr',
